...
function matchCountyName(targetCounty, candidateCounty) {
    return normalizeCounty(targetCounty) === normalizeCounty(candidateCounty);
}
function parseJsonFromMaybePrefixedText(raw) {
...
async function tryReadJson(filePath) {
    try {
        ...
        return parseJsonFromMaybePrefixedText(raw);
    }
    catch {
        return null;
    }
...
async function getToolMap() {
    return tryReadJson(TOOL_MAP_PATH);
}
async function getApiCatalog() {
    return tryReadJson(API_CATALOG_PATH);
}
async function getPortalDiscovery() {
    return tryReadJson(PORTAL_DISCOVERY_PATH);
}
async function getSubmissionStore() {
    const store = await tryReadJson(SUBMISSIONS_PATH);
    return store ?? { generatedAt: nowIso(), records: [] };
}
async function saveSubmissionStore(store) {
    await mkdir(REPORTS_DIR, { recursive: true });
    await writeFile(SUBMISSIONS_PATH, `${JSON.stringify(store, null, 2)}\n`, "utf8");
}
function summarizeCapability(countyMap, tool) {
    if (!countyMap)
        return "unavailable";
...
function deterministicChecksum(input) {
    let hash = 0;
...
export async function searchPermits(input) {
    ...
    const candidateCounties = toolMap?.counties.filter((county) => {
        if (!input.county)
            return true;
    ...
    const permits = candidateCounties
        .flatMap((county) => {
        const countyCatalog = apiCatalog?.entries.find((entry) => matchCountyName(entry.county, county.county));
        const countyPortal = portalDiscovery?.results.find((result) => matchCountyName(result.county, county.county));
    ...
    const notes = [];
    if (!toolMap) {
        notes.push("Tool mapping report not found. Run `bun run catalog:map-tools > reports/county_mcp_tool_mapping.json`.");
    }
    if (!apiCatalog) {
        notes.push("API catalog report not found. Run `bun run catalog:build > reports/county_api_catalog.json`.");
    }
    if (permits.length === 0) {
...
export async function getPermitStatus(input) {
    ...
    const countyMap = toolMap?.counties.find((county) => matchCountyName(county.county, input.county));
    const storedSubmission = submissions.records.find((record) => record.permit_number === input.permit_number &&
        matchCountyName(record.county, input.county));
    const capability = summarizeCapability(countyMap, "get_permit_status");
    const status = storedSubmission?.submission_status === "accepted_for_processing"
        ? "submitted"
        : capability === "available"
            ...
            : capability === "limited"
                ? "integration_partial"
                : "unknown";
    const stage = storedSubmission?.submission_status === "accepted_for_processing"
        ? "intake"
        : capability === "available"
    ...
    if (!countyMap) {
        ...
        notes.push("County status endpoint coverage is limited. Confirm with county portal until live polling is added.");
    }
    return {
        ok: true,
        ...
        data: {
            ...
            status,
            stage,
            outstanding_comments: storedSubmission?.submission_status === "rejected_validation"
...
export async function getCountyRequirements(input) {
    ...
    const countyMap = toolMap?.counties.find((county) => matchCountyName(county.county, input.county));
    const countyCatalog = apiCatalog?.entries.find((entry) => matchCountyName(entry.county, input.county));
    ...
    if (normalizedPermit.includes("electrical"))
        baseChecklist.push("Load calculation sheet");
    if (normalizedPermit.includes("roof"))
        baseChecklist.push("Roof product approval / NOA");
    if (normalizedWorkType.includes("new"))
        baseChecklist.push("Site plan and setbacks");
    if (normalizedWorkType.includes("remodel"))
        baseChecklist.push("Existing conditions photos");
    const endpointHints = countyMap?.endpoints
    ...
    const notes = [];
    if (!countyMap) {
        notes.push("County not found in tool mapping report. Requirements are generic baseline and should be confirmed manually.");
    }
...
export async function getPermitDocuments(input) {
    const [toolMap, submissions] = await Promise.all([getToolMap(), getSubmissionStore()]);
    const countyMap = toolMap?.counties.find((county) => matchCountyName(county.county, input.county));
    const record = submissions.records.find((submission) => submission.permit_number === input.permit_number &&
...
export async function submitPermitPackage(input) {
    ...
    const submissionStatus = validationErrors.length > 0
        ? "rejected_validation"
    ...
    const persistedRecord = {
        ...
        documents: input.documents.map((document) => ({
            ...
            checksum: deterministicChecksum(`${document.name}:${document.mime_type}:${document.url ?? ""}:${document.content_base64 ?? ""}`)
        }))
    };
    const existingIndex = submissions.records.findIndex((record) => record.permit_number === input.permit_number &&
        matchCountyName(record.county, input.county));
    if (existingIndex >= 0) {
